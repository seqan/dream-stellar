...
struct StellarOptions : public EPSMatchOptions, public IndexOptions, public VerifierOptions, public DREAMOptions {
    ...
    static constexpr size_t pigeonholeLemma(size_t sequenceLength, size_t errors)
    {
        ...
        using difference_t = dream_stellar::utils::fraction::difference_t;
        return ceil(dream_stellar::utils::fraction{static_cast<difference_t>(sequenceLength - errors), errors + 1});
    }

    static constexpr size_t minLengthWithExactError(size_t absoluteError, dream_stellar::utils::fraction epsilon)
    {
        if (epsilon.numerator() == 0)
            return std::numeric_limits<size_t>::max();

        using difference_t = dream_stellar::utils::fraction::difference_t;
        return ceil(dream_stellar::utils::fraction{static_cast<difference_t>(absoluteError), 1} / epsilon);
    }

    static constexpr size_t absoluteErrors(dream_stellar::utils::fraction epsilon, size_t sequenceLength)
    {
        using difference_t = dream_stellar::utils::fraction::difference_t;
        return floor(dream_stellar::utils::fraction{static_cast<difference_t>(sequenceLength), 1} * epsilon);
    }
